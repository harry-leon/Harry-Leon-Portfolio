...
export default function Breadcrumbs() {
  ...
  // Generate initials from the header display name for the mobile view.
  const initials = getInitials(headerDisplayName)
  ...
  return (
    <div className="flex items-center gap-2 min-w-0 text-lg text-black dark:text-white my-auto">
      <Link
        href="/"
        className={cn(
          "shrink-0 font-semibold text-gray-900 dark:text-gray-100",
          "transition-colors duration-300 ease-out hover:text-accent-500 dark:hover:text-accent-400",
          "active:scale-95 focus:outline-none focus-visible:text-accent-500 dark:focus-visible:text-accent-400"
        )}
      >
        {/* Initials on mobile */}
        <span className="block md:hidden text-lg">{initials}</span>
        {/* Full name on desktop */}
        <span className="hidden md:inline text-lg">{headerDisplayName}</span>
      </Link>
